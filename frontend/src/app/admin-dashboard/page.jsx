              ...
              <div className="space-y-4">
                {data.pending.length === 0 ? (
                  ...
                  data.pending.map(item => (
                    <div key={item.id} className="bg-[#ffffdd] border border-[#e0e0b7] p-4 rounded">
                      <h4 className="font-bold text-[#4a772e]">{item.title}</h4>
                      <p className="text-sm text-gray-700">Author: {item.author_name}</p>
                      <p className="text-sm text-gray-700">Type: {item.submission_type}</p>
                      <div className="mt-3 flex gap-2">
                        <button onClick={() => handleAction(item.id, 'approve')} className="px-4 py-1 bg-[#4a772e] text-white rounded text-sm font-bold">
                          Approve
                        </button>
                        <button onClick={() => setSelectedId(item.id)} className="px-4 py-1 bg-[#cc5555] text-white rounded text-sm font-bold">
                          Reject
                        </button>
                      </div>
                      {selectedId === item.id && (
                        <div className="mt-3">
                          ...
                          <div className="mt-2 flex gap-2">
                            <button onClick={() => handleAction(item.id, 'reject')} className="px-3 py-1 bg-red-600 text-white rounded text-xs">
                              Send
                            </button>
                            <button onClick={() => { setSelectedId(null); setFeedbackInput(''); }} className="px-3 py-1 bg-gray-400 text-white rounded text-xs">
                              Cancel
                            </button>
                          </div>
                        </div>
